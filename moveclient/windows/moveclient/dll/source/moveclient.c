...
LPMoveStateDeferred slpMoveStateDeferred;
...
static __inline int sendData(uint8_t *data_pointer, uint32_t data_size) {
  ...
  while (0 < data_size) {
	int bytes = send(sControlSocket, (void *)data_pointer, data_size, 0);
...
BOOL WINAPI DllMain(HINSTANCE hInstDLL, DWORD fdwReason, LPVOID lpvReserved) {
  ...
  switch (fdwReason) {
    ...
    case DLL_PROCESS_DETACH:
      // Unload dll due to process termination or FreeLibrary
      break;
...
int startTransfer(void) {
  ...
  if ((shUpdateMoveState = CreateThread(NULL, 0, &UpdateMoveState, slpMoveStateDeferred, 0, NULL)) == NULL) {
    stopTransfer();
...
// PS3 Connect
__declspec(dllexport) INT __cdecl Connect(PCSTR lpRemoteAddress, PCSTR lpPort, LPMoveStateDeferred lpMoveStateDeferred) {
  WSADATA wsaData;
  ...
  slpMoveStateDeferred = lpMoveStateDeferred;
...
// PS3 Disconnect
__declspec(dllexport) INT __cdecl Disconnect() {
...
__declspec(dllexport) INT __cdecl Pause(void) {
...
__declspec(dllexport) INT __cdecl Resume(void) {
...
__declspec(dllexport) INT __cdecl PauseCamera(void) {
...
__declspec(dllexport) INT __cdecl ResumeCamera(void) {
...
__declspec(dllexport) INT __cdecl Rumble(uint32_t gem_num, uint32_t rumble) {
...
__declspec(dllexport) INT __cdecl ForceRGB(uint32_t gem_num, float r, float g, float b) {
...
__declspec(dllexport) INT __cdecl TrackHues(uint32_t req_hue_gem_0, uint32_t req_hue_gem_1, uint32_t req_hue_gem_2, uint32_t req_hue_gem_3) {
...
__declspec(dllexport) INT __cdecl PrepareCamera(uint32_t max_exposure, float image_quality) {
...
__declspec(dllexport) INT __cdecl CameraSetNumSlices(uint32_t slices) {
...
// Recv from PS3
INT WINAPI UpdateMoveState(LPMoveStateDeferred lpMoveStateDeferred) {
  ...
  LPMoveServerPacketHeader lpMoveServerPacketHeader;
  TIMEVAL timeout;
  ...
  while (sTransferring) {
    ...
    if (0 < returnCode) {  // Check file descriptors
      ...
      if (FD_ISSET(sTransferSocket, &fd_read)) {
        ...
        if (bytes == SOCKET_ERROR) {  // Recv error occurred
          ...
          if (lastError != WSAEWOULDBLOCK && sTransferring) {  // Ignore would-block
            lpMoveStateDeferred->fpUpdateFailure(lastError);

          }

        } else {
          // Get pointer to header in the buffer
          lpMoveServerPacketHeader = (LPMoveServerPacketHeader) &buffer[0];
          ...
          if (lpMoveServerPacketHeader->packet_code == MOVE_PACKET_CODE_STANDARD) {
            memcpy_s(lpMoveStateDeferred->lpMoveServerPacket, packet_size, (void *)lpMoveServerPacketHeader, packet_size);

            // Deserialize packet
            deserializeMoveServerPacket(lpMoveStateDeferred->lpMoveServerPacket);

            // Callback deferred
            if (returnCode = lpMoveStateDeferred->fpUpdateSuccess(lpMoveStateDeferred->lpMoveServerPacket)) {
              lpMoveStateDeferred->fpUpdateFailure(returnCode);

            }

          }else if (lpMoveServerPacketHeader->packet_code == MOVE_PACKET_CODE_CAMERA_FRAME_SLICE) {
            memcpy_s(lpMoveStateDeferred->lpMoveServerCameraFrameSlicePacket, cameraPacketSize, (void *)lpMoveServerPacketHeader, cameraPacketSize);

            // Callback deferred
            if (returnCode = lpMoveStateDeferred->fpUpdateCameraSuccess(lpMoveStateDeferred->lpMoveServerCameraFrameSlicePacket)) {
              lpMoveStateDeferred->fpUpdateCameraFailure(returnCode);
      ...
      if (FD_ISSET(sTransferSocket, &fd_except)) {
        ...
        if (sTransferring) {
          lpMoveStateDeferred->fpUpdateFailure(WSAGetLastError());
